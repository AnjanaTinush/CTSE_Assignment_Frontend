...
const selectClass =
  "w-full appearance-none rounded-2xl border border-line bg-white px-4 py-3 text-sm text-label outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10 cursor-pointer";
...
const PRIORITY_CONFIG = {
  NORMAL: { label: "Normal",  color: "text-word bg-white border-line" },
  HIGH:   { label: "High",    color: "text-warning bg-warning/5 border-warning/20" },
  URGENT: { label: "Urgent",  color: "text-danger bg-danger/5 border-danger/20" },
};
...
const AssignDeliveryDrawer = ({
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-sm bg-white border-l border-line shadow-2xl">
        <div className="flex h-full flex-col">

          {/* Header */}
          <div className="border-b border-line bg-[#fcfaf6] px-6 py-5">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-word">Delivery</p>
            <h2 className="mt-1 text-2xl font-semibold tracking-[-0.02em] text-label">Assign Delivery</h2>
            <p className="mt-1 text-sm text-word/70">Link a pending order to a delivery user.</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-2xl border px-3 py-2.5 text-xs font-semibold transition ${
                        active ? cfg.color + " ring-2 ring-offset-1 ring-primary/20" : "border-line bg-white text-word/60 hover:bg-[#f5f0ea]"
                      }`}
                    >
                      {cfg.label}
                    </button>
            ...
            <div className="flex flex-col gap-2">
              <label className={labelClass}>
                Estimated Delivery Time <span className="normal-case tracking-normal text-word/40">(optional)</span>
              </label>
              <input
                type="datetime-local"
            ...
            <div className="flex flex-col gap-2">
              <label className={labelClass}>
                Notes <span className="normal-case tracking-normal text-word/40">(optional)</span>
              </label>
              <textarea
            ...
            {deliveryAssignForm.orderId && deliveryAssignForm.deliveryUserId && (
              <div className="rounded-[22px] border border-success/20 bg-success/5 px-4 py-3.5">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-success">Ready to assign</p>
